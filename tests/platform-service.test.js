...
test('dashboard view exposes Microsoft, YubiKey, Keycloak, Okta, and generic federation platforms', () => {
  ...
  assert.deepEqual(
    ...
    ['microsoft-verified-id', 'yubikey-fido2', 'keycloak', 'okta', 'generic-oidc-saml']
  );
...
test('YubiKey platform has a readiness review step', () => {
  const platform = getPlatformDefinition('yubikey-fido2');
  const testStep = platform.steps.find((step) => step.testStep);

  assert.equal(testStep.id, 'test');
  assert.equal(platform.summary.includes('YubiKey 5C NFC'), true);
});

test('all setup wizard fields include tooltip help text', () => {
  for (const platformId of ['microsoft-verified-id', 'yubikey-fido2', 'keycloak', 'okta', 'generic-oidc-saml']) {
