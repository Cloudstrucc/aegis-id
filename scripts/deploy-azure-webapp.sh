...
TENANT_KEYS=(
  ...
  WALLET_RECOVERY_REQUEST_STORE_PATH NOTIFICATION_SETTINGS_STORE_PATH
  NOTIFICATION_LOG_STORE_PATH PASSWORD_RESET_STORE_PATH SIGN_IN_METHOD_STORE_PATH
  ACCOUNT_RECOVERY_CODE_STORE_PATH EMAIL_VERIFICATION_STORE_PATH ACCOUNT_REENROLMENT_STORE_PATH
  REGISTRATION_CODE_STORE_PATH CHECKOUT_SESSION_STORE_PATH ORGANIZATION_IDENTITY_STORE_PATH ROOT_WALLET_STORE_PATH ROOT_WALLET_POLICY_ENFORCED BREAK_GLASS_STORE_PATH
  STRIPE_SECRET_KEY STRIPE_WEBHOOK_SECRET STRIPE_PUBLISHABLE_KEY
  ARIES_ORG_INVITATION_MODE CREDENTIAL_ASSURANCE_MODE CREDENTIAL_ASSURANCE_HIGH_SIGNALS
  CREDENTIAL_ASSURANCE_HIGH_ROLE_PATTERN CREDENTIAL_ASSURANCE_DEFAULT
  AUDIT_CHAIN_ENABLED AUDIT_SIGNING_ENABLED AUDIT_SIGNING_MODE AUDIT_SIGNING_KEYVAULT_KEY_ID
  AUDIT_ANCHOR_MODE AUDIT_ANCHOR_CONTAINER AUDIT_ANCHOR_INTERVAL_SECONDS
  # Both default into wwwroot, which is replaced on every deploy — the signing
  # key and the anchor heads have to be on the /home mount to survive one.
  AUDIT_SIGNING_LOCAL_KEY_PATH AUDIT_ANCHOR_DIR
  IOS_APP_BUNDLE_IDS DIGITAL_SIGNATURE_APP_URL WALLET_URL_SCHEME
  APPROVER_RECOVERY_STORE_PATH
  OIDC_CODE_STORE_PATH OIDC_WALLET_SIGNIN_STORE_PATH WALLET_CHALLENGE_STORE_PATH WALLET_PASSKEY_STORE_PATH AUDIT_STORE_PATH
  CONNECTED_APP_STORE_PATH CONNECTED_APP_LOG_STORE_PATH CONNECTED_APP_OAUTH_CODE_STORE_PATH
  CONNECTED_APP_SIGNING_KEY_STORE_PATH CONNECTED_APP_UPSTREAM_STATE_STORE_PATH CONNECTED_APP_SECRET_ENCRYPTION_KEY
  CONNECTED_APP_UPSTREAM_IDP_MODE CONNECTED_APP_UPSTREAM_ENTRA_TENANT_ID CONNECTED_APP_UPSTREAM_ENTRA_CLIENT_ID
  CONNECTED_APP_UPSTREAM_ENTRA_CLIENT_SECRET CONNECTED_APP_UPSTREAM_ENTRA_REDIRECT_URI CONNECTED_APP_UPSTREAM_ENTRA_SCOPES
  CONNECTED_APP_UPSTREAM_ENTRA_ISSUER CONNECTED_APP_UPSTREAM_ENTRA_AUTHORIZATION_ENDPOINT
  CONNECTED_APP_UPSTREAM_ENTRA_TOKEN_ENDPOINT CONNECTED_APP_UPSTREAM_ENTRA_JWKS_URI
  ...
  AEGIS_DID_WEB_DID_DOCUMENT_URL AEGIS_DID_WEB_CONFIGURATION_URL AEGIS_DID_WEB_KEY_NAME
  AEGIS_DID_WEB_KEY_ALG AEGIS_DID_WEB_KEY_CURVE AEGIS_DID_WEB_KEYVAULT_URL AEGIS_DID_WEB_KEYVAULT_KEY_ID
