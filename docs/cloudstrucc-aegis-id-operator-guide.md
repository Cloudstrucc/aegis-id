...
![Cloudstrucc Aegis ID architecture cover](../public/images/cloudstrucc-identity-architecture.png)

# Cloudstrucc Inc. Operator Guide
...
Cloudstrucc Aegis ID is a dual-track identity service for Cloudstrucc Inc.
...
```mermaid
flowchart LR
    Visitor["Anonymous visitor"] --> Subscribe["Subscribe on Cloudstrucc landing page"]
...
![Cloudstrucc setup walkthrough poster](../public/images/setup-walkthrough-poster.png)
...
sequenceDiagram
    ...
    participant Home as Cloudstrucc landing page
...
<details>
<summary><strong>Suggested Cloudstrucc Inc. pilot claim set</strong></summary>
...
6. Scan the QR displayed by Cloudstrucc Aegis ID.
...
- [ ] Cloudstrucc Inc. Azure tenant exists.
...
```mermaid
flowchart TB
    Admin["Cloudstrucc admin"] --> Entra["Entra admin center"]
    ...
    API --> Aegis["Cloudstrucc Aegis ID live adapter"]
...
Use YubiKey for phishing-resistant sign-in to Cloudstrucc-controlled admin and subscriber access, Entra admin work, Keycloak, Okta, or other SSO layers.
...
- [ ] Scope the policy to a Cloudstrucc pilot group first.
- [ ] Register a YubiKey for a pilot user.
- [ ] Confirm the pilot user can sign in with the YubiKey.
- [ ] Add Conditional Access authentication strength for sensitive Cloudstrucc admin paths.
...
Do not use the Aries lab as a production dependency for the Microsoft-native Cloudstrucc path.

---

## Azure Free-Tier Pilot Deployment

This app can start on Azure App Service Free `F1` for a Cloudstrucc Inc. pilot if you keep it simple:
